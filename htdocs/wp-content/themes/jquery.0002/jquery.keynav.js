...
//    var currentScroll = $(window).scrollTop(); 
  ...
  $.keynav.goDown = function () {
	  ...
	  $.keynav.activateClosest(cur,quad);
    var currentHeight = $(cur).outerHeight(), offset = $(cur).offset();
		if ( offset.top + currentHeight > $(window).scrollTop() + $(window).height() ) {
			$(window).scrollTop( offset.top + currentHeight );
		}
  }

  //new activate function
  $.keynav.activate = function() {
    window.location = $('.withfocus').children().find('a').attr('href');
  }
  /*
  //original activate function
    $.keynav.activate = function () {
  	  var kn = $.keynav;
